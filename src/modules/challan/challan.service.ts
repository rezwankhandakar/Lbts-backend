...
import { assertCanChangeBatch, assertCanDelete, assertCanEdit } from './challan.access'
import {
...
async function resolveActorNames(
  records: { createdBy: unknown; submittedBy?: unknown; updatedBy?: unknown }[],
...
function buildListFilter(query: ListChallansQuery): QueryFilter<Challan> {
  ...
  if (query.customer) {
    clauses.push({ customerName: new RegExp(escapeRegex(query.customer), 'i') })
  }
  ...
  if (query.model) {
    clauses.push({ 'items.productModel': new RegExp(escapeRegex(query.model), 'i') })
  }
  if (query.zonePo) {
...
export async function listChallans(query: ListChallansQuery): Promise<ListChallansResult> {
  const filter = buildListFilter(query)
  const skip = (query.page - 1) * query.limit

  const [records, totals] = await Promise.all([
    ChallanModel.find(filter).sort({ createdAt: -1 }).skip(skip).limit(query.limit),
    totalsFor(filter),
  ])
...
export async function getChallanStats(): Promise<ChallanStats> {
  ...
  const [totals, todayCount, batchRows] = await Promise.all([
    totalsFor({}),
    ChallanModel.countDocuments({ submittedAt: { $gte: today, $lt: tomorrow } }),
 ...
 */
export async function suggestChallanValues(query: ChallanSuggestionQuery): Promise<string[]> {
  ...
  const rows = await ChallanModel.aggregate<{ _id: string; count: number }>([
     ...
     */
    ...(path.startsWith('items.') ? [{ $unwind: '$items' }, { $match: { [path]: prefix } }] : []),
...
export async function listChallanBatches(query: ListBatchesQuery): Promise<ListBatchesResult> {
  ...
  const [records, total] = await Promise.all([
    ChallanBatchModel.find(filter).sort({ createdAt: -1 }).skip(skip).limit(query.limit),
...
export async function getChallanBatch(id: string): Promise<ChallanBatchDetail> {
  ...
  const challans = await ChallanModel.find({ batchId: batch._id }).sort({ sourcePageStart: 1 })
...
async function refreshBatchProgress(batchId: unknown): Promise<void> {
  ...
  const isComplete = batch.sourcePageCount > 0 && accounted.size === batch.sourcePageCount

  batch.set('skippedPages', [...new Set(skipped)].sort((a, b) => a - b))
...
export async function setBatchSkippedPages(
  ...
  const outOfBounds = wanted.filter((page) => page < 1 || page > batch.sourcePageCount)
  ...
  if (conflict !== undefined) {
    const owner = claimed.find((range) => conflict >= range.startPage && conflict <= range.endPage)
    throw new AppError(
      409,
...
export async function findDuplicateChallans(
  ...
  if (query.sessionKey && query.customerName) {
    ...
    if (batch) {
      ...
      const matches = await ChallanModel.find(filter).sort({ createdAt: -1 }).limit(5)
...
function normalizeFields(input: SubmitChallanInput | UpdateChallanInput): NormalizedFields {
  ...
  return {
    ...
    senderMobile: input.senderMobile ? normalizeMobile(input.senderMobile) : null,
 ...
 */
async function claimSubmission(submissionKey: string, actor: UserDocument): Promise<void> {
  try {
...
async function releaseSubmission(submissionKey: string): Promise<void> {
  try {
    await ChallanSubmissionModel.deleteOne({ _id: submissionKey, status: 'pending' })
  } catch (error) {
...
export async function submitChallan(
  ...
  const range = { startPage: input.sourcePageStart, endPage: input.sourcePageEnd }
...
export async function getSubmittedChallan(challanId: string): Promise<SubmitChallanResult> {
 ...
 */
export async function removeChallan(id: string, actor: UserDocument): Promise<{ id: string }> {
  const challan = await findChallan(id)
 ...
 * belongs to.
 */
export async function readChallanDocumentStream(id: string): Promise<ChallanDownload> {
  const challan = await findChallan(id)
...
export async function buildBatchPdf(id: string): Promise<BatchDownload> {
  ...
  if (batch.status !== 'Completed') {
    const remaining = Math.max(batch.sourcePageCount - batch.assignedPageCount, 0)
    throw new AppError(
      409,
  ...
  const safeName = batch.sourceFileName.replace(/\.pdf$/i, '').replace(/[^A-Za-z0-9._-]+/g, '-')
