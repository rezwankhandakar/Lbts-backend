...
import { BarcodePayloadError, challanBarcodePayload, encodeCode128B } from './lib/code128'
import {
  MAX_BACK_PAGE_ITEMS,
  backPageLayout,
...
describe('Code 128-B', () => {
  ...
  it('carries the challan number itself, and nothing derived from it', () => {
    assert.equal(challanBarcodePayload('LBTS-CH-2026-000001'), 'LBTS-CH-2026-000001')
  })
  ...
  it('keeps a real challan number inside a printable width', () => {
    ...
    assert.ok(moduleWidthMm > 0.25, `module width ${moduleWidthMm}mm is too narrow to scan`)
  })
})
...
describe('winAnsiSafe', () => {
  ...
  it('collapses newlines so a pasted address stays on one line', () => {
    assert.equal(winAnsiSafe('House 12\nRoad 4\nBlock C'), 'House 12 Road 4 Block C')
  })
})
...
async function makeSourcePages(count: number, marker: string): Promise<Uint8Array> {
...
describe('the back page layout', () => {
  ...
  it('keeps every band inside an A4 sheet, in order', () => {
    ...
    assert.ok(layout.barcodeBaseline + 76 < 841.89, 'the barcode runs off the top')
    assert.ok(layout.identifierY < layout.barcodeBaseline, 'identifiers overlap the barcode')
    assert.ok(layout.detailTop < layout.identifierY, 'details overlap the identifiers')
    ...
    assert.ok(layout.footerY < layout.detailBottom, 'the footer overlaps the last row')
  })
  ...
  it('keeps a long goods table inside the page', () => {
    ...
    assert.ok(layout.itemsTop < layout.detailBottom, 'goods overlap the details')
    assert.ok(layout.itemsBottom > 0, 'the goods table runs off the bottom')
    assert.ok(layout.footerY < layout.itemsBottom, 'the footer overlaps the total line')
    assert.ok(layout.footerY > 6, 'the footer sits inside the bottom bar')
  })
...
describe('the individual challan document', () => {
  it('is the original pages followed by the back page', async () => {
    ...
    const final = await generateChallanFinalPdf({ frontPages: front, backPage: back })
  ...
  it('appends one page whatever the challan is long', async () => {
    for (const pages of [1, 3, 5]) {
      ...
      assert.equal(await readPageCount(final), pages + 1, `${pages} front pages`)
    }
  })
...
describe('regenerating a corrected challan', () => {
  it('replaces the back page and leaves the original pages alone', async () => {
    ...
    const corrected = await replaceChallanBackPage(
      ...
      await generateChallanBackPage({
        ...
        items: [{ productName: 'Refrigerator', model: 'WFA-2D4-GDEH-XX', qty: 9 }],
      }),
    )
  ...
  it('can be repeated without the document growing a page each time', async () => {
    ...
    for (let round = 0; round < 3; round += 1) {
      document = await replaceChallanBackPage(
        ...
        await generateChallanBackPage({
          ...BACK_PAGE,
          items: [{ productName: 'Refrigerator', model: 'WFA-2D4-GDEH-XX', qty: round + 1 }],
        }),
      )
    }
  ...
  it('refuses a stored document that has lost its pages', async () => {
    ...
    await assert.rejects(() => replaceChallanBackPage(onePage, back), /missing its pages/)
  })
})
...
describe('the completed batch document', () => {
  it('is every challan document, in the order it was given', async () => {
    ...
    const second = await generateChallanFinalPdf({
      ...
      backPage: await generateChallanBackPage({ ...BACK_PAGE, slNumber: 10235 }),
    })
    const third = await generateChallanFinalPdf({
      frontPages: await makeSourcePages(3, 'THIRD'),
      backPage: await generateChallanBackPage({ ...BACK_PAGE, slNumber: 10236 }),
    })
...
describe('reading a PDF this API did not produce', () => {
  ...
  it('refuses bytes that are not a PDF, with something an operator can read', async () => {
    await assert.rejects(
      () => readPageCount(new Uint8Array(Buffer.from('not a pdf at all')), 'challan pages'),
