...
import { batchProgress, checkRangeAgainst, pageCountOf, unassignedRanges } from './lib/page-ranges'
...
function backPageFor(slNumber: number, challanNumber: string): ChallanBackPageData {
  return {
    slNumber,
    challanNumber,
...
describe('Scenario A — three challans out of one PDF, all filed', () => {
  it('produces three documents, three identifiers, three barcodes and one batch', async () => {
    ...
    for (const [index, range] of ranges.entries()) {
      ...
      const challan = await fileChallan(source, range.startPage, range.endPage, index + 1)
      filed.push(challan)
    ...
    const symbols = filed.map((c) => encodeCode128B(c.challanNumber).codes.join(','))
...
describe('Scenario B — five of ten filed, then the workspace closes', () => {
  it('leaves the batch incomplete and names the pages that are missing', async () => {
    ...
    for (const [index, range] of filedRanges.entries()) {
      documents.push(
        (await fileChallan(source, range.startPage, range.endPage, index + 1)).document,
      )
    }
    ...
    assert.deepEqual(unassignedRanges(filedRanges, 10), [{ startPage: 6, endPage: 10 }])
  })
})
...
describe('Scenarios C and D — Bangla text on the way into storage', () => {
  ...
  it('keeps a Bangla customer comparable for duplicate detection', () => {
    ...
    assert.equal(comparisonKey('মোঃ আরিফ হোসেন'), comparisonKey('মোঃ  আরিফ  হোসেন!'))
  })
})
