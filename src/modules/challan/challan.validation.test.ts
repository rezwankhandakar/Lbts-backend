...
const VALID = {
  ...
  senderMobile: '',
  zonePo: 'Zone-7',
  items: JSON.stringify([{ productName: 'Refrigerator', model: 'WFA-2D4-GDEH-XX', qty: '2' }]),
...
describe('submitting a challan', () => {
  ...
  it('reads the acknowledgement as a boolean, not as a truthy string', () => {
    ...
    assert.equal(submitChallanSchema.parse({ ...VALID, acknowledgeDuplicate: 'false' }).acknowledgeDuplicate, false)
    assert.equal(submitChallanSchema.parse({ ...VALID, acknowledgeDuplicate: 'true' }).acknowledgeDuplicate, true)
    assert.equal(submitChallanSchema.parse({ ...VALID, acknowledgeDuplicate: '1' }).acknowledgeDuplicate, true)
    assert.equal(submitChallanSchema.parse({ ...VALID, acknowledgeDuplicate: 'yes' }).acknowledgeDuplicate, false)
  })

  it('normalises a receiver mobile written any of the usual ways', () => {
    for (const written of ['01712345678', '01712-345678', '+8801712345678', '8801712345678']) {
      assert.equal(
        submitChallanSchema.parse({ ...VALID, receiverMobile: written }).receiverMobile,
        '01712345678',
      )
    }
  })
  ...
  it('refuses something that is not a contact number at all', () => {
    assert.equal(submitChallanSchema.safeParse({ ...VALID, receiverMobile: 'call the shop' }).success, false)
  })
  ...
  it('refuses a session key that is not one', () => {
    for (const key of ['', 'short', '../../etc/passwd', 'has spaces here']) {
      assert.equal(submitChallanSchema.safeParse({ ...VALID, sessionKey: key }).success, false, key)
    }
  })

  it('refuses a submission with no idempotency key', () => {
    assert.equal(submitChallanSchema.safeParse({ ...VALID, submissionKey: '' }).success, false)
  })
  ...
  it('treats the sender number and the zone/PO as optional', () => {
    const parsed = submitChallanSchema.parse({ ...VALID, senderMobile: '', zonePo: '' })
...
describe('the product rows on a submission', () => {
  const rows = (items: unknown[]) => ({ ...VALID, items: JSON.stringify(items) })
  ...
  it('refuses a row missing its product or model', () => {
    assert.equal(
      submitChallanSchema.safeParse(rows([{ productName: '', model: 'WFA', qty: '1' }])).success,
      false,
    )
    assert.equal(
      submitChallanSchema.safeParse(rows([{ productName: 'Refrigerator', model: '', qty: '1' }]))
        .success,
      false,
    )
  })
    ...
    for (const qty of ['0', '-2', '2.5', 'two', '']) {
      assert.equal(
        submitChallanSchema.safeParse(
          rows([{ productName: 'Refrigerator', model: 'WFA-2D4-GDEH-XX', qty }]),
        ).success,
        false,
        qty,
      )
  ...
  it('refuses a field that is not readable as rows at all', () => {
    assert.equal(submitChallanSchema.safeParse({ ...VALID, items: 'not json' }).success, false)
    assert.equal(submitChallanSchema.safeParse({ ...VALID, items: '{}' }).success, false)
  })
})
...
describe('correcting a challan', () => {
  it('takes the values and nothing about where they came from', () => {
    const parsed = updateChallanSchema.parse({
      ...
      senderMobile: '',
      zonePo: '',
      items: [{ productName: 'Refrigerator', model: 'WFA-2D4-GDEH-XX', qty: 3 }],
...
describe('the records list', () => {
  ...
  it('caps the page size, so a crafted query cannot ask for everything', () => {
    assert.equal(listChallansQuerySchema.safeParse({ limit: '5000' }).success, false)
  })

  it('refuses a date range that ends before it starts', () => {
    const result = listChallansQuerySchema.safeParse({ from: '2026-09-10', to: '2026-09-01' })
    assert.equal(result.success, false)
  })
})
...
describe('type-ahead', () => {
  it('only answers for the fields it was told about', () => {
    assert.equal(suggestionQuerySchema.safeParse({ field: 'district', q: 'Dh' }).success, true)
    // An open field name would let a caller enumerate any column it liked.
    assert.equal(suggestionQuerySchema.safeParse({ field: 'receiverMobile', q: '017' }).success, false)
    assert.equal(suggestionQuerySchema.safeParse({ field: 'createdBy', q: 'ad' }).success, false)
  })

  it('will not run on a single letter', () => {
    assert.equal(suggestionQuerySchema.safeParse({ field: 'district', q: 'D' }).success, false)
  })
})
